...
\begin{figure}[htb]
   ...
   \includegraphics[width=\textwidth]{figuras/matriz.jpg}
...
Ao longo do período dado, os retornos das quatro faixas de menor risco e do próprio S\&P~500 concentram-se entre 8\% e 14\% ao ano, com pequena separação entre elas. A faixa Muito Alto isola-se acima de 40\% anualizados, o que indica que o critério proposto identifica, ao longo de 15 anos, um subconjunto de ativos cuja recompensa média excede substancialmente o índice de mercado. A hierarquia esperada da teoria de risco-retorno é preservada: portfólios de maior risco entregam retornos superiores no acumulado do período.
...
O comportamento inverte-se em intensidade em relação à janela calma. A faixa Muito Alto ultrapassa 60\% de retorno anualizado, enquanto as faixas Muito Baixo e Baixo permanecem próximas de zero. O S\&P~500 mantém-se em torno de 11\%, valor consistente com o observado nas demais janelas, mas agora distante da faixa de risco elevado. A leitura é consistente com a evidência de que ativos de maior Beta amplificam movimentos direcionais do mercado segundo \citeonline{Fama1992CrossSection}, apresentando quedas proporcionalmente maiores no momento do choque e recuperação de amplitude também maior na fase subsequente.

O conjunto das três janelas evidencia que a métrica composta desenvolvida no trabalho representa o universo de forma coerente com o comportamento esperado do mercado sob diferentes regimes. Em longos períodos, a hierarquia de retorno por faixa é preservada; em janelas curtas de tranquilidade, ela se achata e o critério perde poder discriminante; em janelas dominadas por choque e recuperação, ela se acentua. A sensibilidade ao regime é evidência de que o critério proposto captura risco de forma suficientemente informativa para separar comportamentos distintos em diferentes contextos macroeconômicos, o que sustenta a apresentação dos recortes seguintes desta seção.
...
O experimento reportado adota um protocolo padronizado, replicado para cada uma das cinco faixas de risco. Para cada faixa, é selecionado o conjunto das dez ações com maior alocação percentual recomendada pelo modelo, sem aplicação dos modificadores de risco, isto é, com os pesos $\alpha_{EV}$, $\alpha_{DY}$ e $\alpha_{EB}$ zerados. Essa configuração permite avaliar o desempenho da lógica de seleção em sua forma mais direta, sem interferência das personalizações discutidas na subseção anterior, e produz um teste comparável entre as faixas.
