\chapter{Projetando o Ambiente Analítico}
\label{metodologia}
...
O dicionário de dados formaliza a documentação do modelo dimensional apresentado nas subseções anteriores, associando a cada atributo persistido no repositório a tabela em que ele reside, sua descrição semântica e o campo de origem no dado bruto do qual ele é derivado. O Quadro \ref{quad:dicionario-dados} apresenta essa consolidação.

{\footnotesize
\captionof{quadro}{Dicionário de dados do modelo dimensional.}
\label{quad:dicionario-dados}
...
O mapeamento de-para consolidado durante a segunda etapa do planejamento é apresentado no Quadro~\ref{quad:mapeamento-etl}. Ele documenta a origem de cada atributo materializado no \textit{data mart}, a transformação aplicada até a carga e a tabela de destino, permitindo a rastreabilidade completa entre o dado bruto e o dado consumido pelas consultas analíticas.

{\footnotesize
\captionof{quadro}{Mapeamento de-para entre os campos do dado bruto e os atributos do modelo dimensional.}
\label{quad:mapeamento-etl}
...
\section{Construção do \textit{Pipeline} ETL}
...
A última etapa do tratamento consiste em remover ativos cujos retornos ou indicadores apresentam valores extremos, suficientemente distantes do restante do universo para distorcer as análises. Esses ativos comprometem o estudo tanto na visualização, tendo em vista que poucos pontos extremos passam a definir a escala dos gráficos comparativos de retorno, comprimindo os demais ativos em uma faixa visualmente indistinta, quanto na padronização via \textit{Z-score} aplicada aos modificadores, dado que valores extremos inflam o desvio padrão populacional e reduzem artificialmente as distâncias relativas entre os ativos típicos.
...
A última etapa da modelagem traduz o \textit{score} contínuo em uma classificação categórica composta por cinco faixas, denominadas Muito Baixo, Baixo, Médio, Alto e Muito Alto, além da categoria de risco desconhecido para caracterizar casos de ativos que ainda não têm dados suficientes disponíveis para o calculo do risco. A motivação para essa categorização é dupla. Sob o ponto de vista analítico, a discretização viabiliza agrupamentos diretos para comparações entre coortes de ativos, como cálculo de retorno médio por faixa, exposição setorial e análise de \textit{drawdown}. Sob o ponto de vista comunicacional, ela aproxima o modelo da linguagem usual da indústria de investimentos, em que perfis de risco são tipicamente expressos em rótulos qualitativos.
...
Muito Baixo & \(\text{risco}_{\text{base}} \leq -0{,}60\) \\ \hline
Baixo & \(-0{,}60 < \text{risco}_{\text{base}} \leq -0{,}25\) \\ \hline
Médio & \(-0{,}25 < \text{risco}_{\text{base}} \leq 0{,}25\) \\ \hline
Alto & \(0{,}25 < \text{risco}_{\text{base}} \leq 0{,}60\) \\ \hline
Muito Alto & \(\text{risco}_{\text{base}} > 0{,}60\) \\ \hline
