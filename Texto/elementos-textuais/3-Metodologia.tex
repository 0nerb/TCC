...
O dicionário de dados formaliza a documentação do modelo dimensional apresentado nas subseções anteriores, associando a cada atributo persistido no repositório a tabela em que ele reside, sua descrição semântica e o campo de origem no dado bruto do qual ele é derivado. O Quadro \ref{tab:dicionario-dados} apresenta essa consolidação.
...
O mapeamento de-para consolidado durante a segunda etapa do planejamento é apresentado no Quadro~\ref{tab:mapeamento-etl}. Ele documenta a origem de cada atributo materializado no \textit{data mart}, a transformação aplicada até a carga e a tabela de destino, permitindo a rastreabilidade completa entre o dado bruto e o dado consumido pelas consultas analíticas.
